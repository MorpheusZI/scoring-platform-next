...
export async function GoogleAISummarizer({ KomitmenAtasan, KomitmenBawahan, Catatan, NamaMentee, NamaManager }: SummaryReq) {
  ...
  const AIResponse = genAIContent.response
...
async function GoogleAIStarChecker({ Judul, Isi }: KomitmenData) {
  ...
  const AIResponseText = genAIRes.response.text()
...
export async function StarChecker(KomitmenDataArr: KomitmenData[]) {
  const promises = KomitmenDataArr.map(async (KomitmenData) => {
